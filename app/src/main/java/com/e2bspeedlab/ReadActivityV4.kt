...
/** Single-line FLASH reader with FAST/THINK and optional auditory frame markers. */
class ReadActivityV4 : Activity() {
    ...
    private lateinit var presenter: FixedFlashPresenterV4
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        clickEnabled = prefs().getBoolean(PREF_CLICK_ENABLED, true)
    ...
    private fun buildUi() {
        ...
        val displayFrame = FrameLayout(this).apply { setBackgroundColor(Color.rgb(12, 15, 20)) }
        ...
        presenter = FixedFlashPresenterV4(
            view = flashText,
            accentColor = Color.rgb(120, 235, 195),
            onUnitShown = { unit -> if (clickEnabled) clickSound.playFor(unit) },
        )
        ...
        clickButton = actionButton("CLICK") { toggleClick() }
        controlRow.addView(fastButton, LinearLayout.LayoutParams(0, dp(46), 1f).apply { marginEnd = dp(4) })
        controlRow.addView(thinkButton, LinearLayout.LayoutParams(0, dp(46), 1f).apply { marginStart = dp(4); marginEnd = dp(4) })
        controlRow.addView(clickButton, LinearLayout.LayoutParams(0, dp(46), 1f).apply { marginStart = dp(4) })
    ...
    private fun installInsets() {
        root.setOnApplyWindowInsetsListener { view, insets ->
            ...
            view.setPadding(dp(18) + safe.left, dp(12) + safe.top, dp(18) + safe.right, dp(12) + safe.bottom)
            insets
        }
    ...
    private fun toggleClick() {
        ...
        prefs().edit().putBoolean(PREF_CLICK_ENABLED, clickEnabled).apply()
        updateClickButton()
    }
    ...
    private fun startGeneration() {
        ...
        presenter.begin()
        val unitizer = ReadUnitizerV4(Locale.getDefault())
    ...
    private fun loadEngine(auto: Boolean) {
        ...
        scope.launch {
            try {
                ...
                statusText.text = profileStatus("READY")
                metricText.text = if (seconds > 0.0) String.format(Locale.US, "loaded %.1fs", seconds) else "READY"
    ...
    private fun showMetrics(info: BenchmarkInfo) {
        metricText.text = String.format(Locale.US, "%.1f tok/s • %.0f ms", info.lastDecodeTokensPerSecond, info.timeToFirstTokenInSecond * 1000.0)
    }
    ...
    private fun updateControls() {
        ...
        clickButton.isEnabled = true // allow muting even while FLASH is running
    ...
    private fun hideKeyboard() {
        (getSystemService(INPUT_METHOD_SERVICE) as InputMethodManager).hideSoftInputFromWindow(promptInput.windowToken, 0)
    }
...
private class ReadUnitizerV4(private val locale: Locale) {
    private val pending = StringBuilder()
    private var carry: String? = null
    private var joinNext = false
    private val japaneseParticles = setOf("は", "が", "を", "に", "へ", "と", "で", "の", "も", "や", "か", "ね", "よ", "ぞ", "さ", "な", "って", "から", "まで", "より")
    ...
    private fun clean(raw: String): String = raw.replace("**", "").replace("__", "").replace("`", "").replace("\n", " ").trim()
    ...
    private fun isKatakanaRun(s: String): Boolean {
        ...
        while (i < s.length) {
            ...
            val valid = cp in 0x30A0..0x30FF || cp in 0x31F0..0x31FF || cp in 0xFF65..0xFF9F || cp == 0x3099 || cp == 0x309A
...
private class FixedFlashPresenterV4(
    private val view: TextView,
    private val accentColor: Int,
    private val onUnitShown: (String) -> Unit,
) {
    ...
    fun enqueue(units: List<String>) {
        ...
        handler.post {
            units.filter { it.isNotBlank() }.forEach { unit -> splitToFit(unit).forEach(queue::addLast) }
            if (!scheduled) step()
        }
    }
    ...
    fun showStatic(text: String) { view.text = text }
    ...
    private fun splitToFit(text: String): List<String> {
        val maxWidth = (view.width - view.paddingLeft - view.paddingRight).takeIf { it > 80 }?.times(0.94f)
    ...
    private fun chooseBreak(text: String, maxWidth: Float): Int {
        ...
        for (i in longestIndex downTo earliestPreferred) {
            val end = offsets[i]
            if (i < offsets.lastIndex && !badStart(text.codePointAt(offsets[i])) && goodBoundary(text, end)) return end
        }
        var safeIndex = longestIndex
        while (safeIndex > 1 && safeIndex < offsets.lastIndex && badStart(text.codePointAt(offsets[safeIndex]))) safeIndex--
    ...
    private fun badStart(cp: Int): Boolean = cp in setOf(
        ...
        'ャ'.code, 'ュ'.code, 'ョ'.code, 'ッ'.code, 'ヮ'.code, 'ヵ'.code, 'ヶ'.code, 0x3099, 0x309A,
    )
