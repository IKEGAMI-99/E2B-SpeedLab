...
/**
 * Practical, deliberately non-chat UI for the tuned E2B path.
 *
 * FLASH = RSVP-style one-unit-at-a-time reading.
 * FLOW  = continuous right-to-left marquee fed directly from the model stream.
 */
class PracticalActivity : Activity() {

    companion object {
        private const val PICK_MODEL_REQUEST = 7101
    ...
    private lateinit var flowView: FlowTextView
    ...
    private lateinit var flashPresenter: FlashPresenter
    ...
    private fun buildUi() {
        ...
        flowView = FlowTextView(this).apply {
            visibility = View.GONE
        }
        displayFrame.addView(flashText, FrameLayout.LayoutParams(
        ...
        flashPresenter = FlashPresenter(flashText, Color.rgb(120, 235, 195))
    ...
    private fun applyMode(newMode: DisplayMode) {
        ...
        flashText.visibility = if (flash) View.VISIBLE else View.GONE
    ...
    private fun updatePace(value: Int) {
        ...
        // FLASH: 240 -> 55 ms/unit. FLOW: 220 -> 1120 px/s.
    ...
    private fun startGeneration() {
        ...
        val unitizer = StreamingUnitizer(Locale.getDefault())
    ...
    private fun loadTurboEngine(auto: Boolean = false) {
        ...
        // The verified POCO profile is the practical default for this app. Persisted Turbo values
        // override it when they belong to the current model.
        val context = if (profileMatches) {
            prefs().getInt(PREF_TURBO_CONTEXT, VERIFIED_CONTEXT)
        } else VERIFIED_CONTEXT
        val fastCpus = if (profileMatches) {
            prefs().getInt(PREF_TURBO_FAST_CPUS, VERIFIED_FAST_CPUS)
        } else VERIFIED_FAST_CPUS
        ...
        scope.launch {
            try {
                ...
                if (mode == DisplayMode.FLASH) flashText.text = "READY"
            } catch (t: Throwable) {
...
/** Incremental ICU word segmentation. The final segment is held until more stream text arrives. */
private class StreamingUnitizer(private val locale: Locale) {
    ...
    private fun drain(final: Boolean): List<String> {
        ...
        for (i in 0 until processCount) {
            ...
            val cleaned = raw
                ...
                .replace("`", "")
                .trim()
            if (cleaned.isNotEmpty() && cleaned.any { !it.isWhitespace() }) out += cleaned
        }
...
/** RSVP presenter with a fixed gaze point: the middle character is accented. */
private class FlashPresenter(
    ...
    fun finishInput() {
        handler.post {
            accepting = false
            if (!scheduled && queue.isEmpty()) target.text = "✓"
        }
    }
    ...
    private fun step() {
        ...
        if (next == null) {
            scheduled = false
            if (!accepting) target.text = "✓"
            return
        }
        scheduled = true
        target.text = accented(next)
        handler.postDelayed({ step() }, intervalMs)
    }

    private fun accented(word: String): CharSequence {
        if (word.length < 2) return word
        val index = (word.length - 1) / 2
        return SpannableString(word).apply {
            setSpan(
                ForegroundColorSpan(accentColor),
                index,
                (index + 1).coerceAtMost(length),
...
/** Streaming single-line marquee that continuously moves generated text right -> left. */
private class FlowTextView(context: Context) : View(context) {
